...
class main_activity():
    ...
    def main(self):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:  # multi CPUs processing
            executor.map(self.real_NB, KPI_names)
        '''
        ...
        eval_file = open('NB_evaluation.txt', 'wt')
        eval_file.writelines(eval_text)
        eval_file.close()
        print('NB Experiment Finished.')

    def real_NB(self, each_KPI):
        ...
        print('========Processing %s NB model training==============' % KPI_name)
        # Train NB model
        nbc = core.nb()
        model = nbc.train_nb(train_data, train_label)
        # Evaluate the NB model
        print('========Processing %s NB model testing===============' % KPI_name)
        pred = nbc.test_nb(model, test_data)
        # Save the model
