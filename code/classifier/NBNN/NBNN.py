...
class main_activity():
    ...
    def main(self):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor: # multi CPUs processing
           executor.map(self.real_NBNN, KPI_names)
        '''
        for each_KPI in KPI_names: # one CPU processing
            self.real_NBNN(each_KPI)
        '''
        ...
        eval_file = open('NBNN_evaluation.txt', 'wt')
        eval_file.writelines(eval_text)
        eval_file.close()
        print('NBNN Experiment Finished.')

    def real_NBNN(self, each_KPI):
        ...
        # Evaluate the NBNN model
        print('========Processing %s NBNN model testing==============' % KPI_name)
        model = core.NBNN()
        pred = model.NBNN_classify(train_data, train_label, test_data)
