...
class main_activity():
    ...
    def main(self):
        ...
        eval_file = open('DT_evaluation.txt', 'wt')
        eval_file.writelines(eval_text)
        eval_file.close()
        print('DT Experiment Finished.')
    ...
    def real_NB(self, each_KPI):
        ...
        print('========Processing %s DT model training==============' % KPI_name)
        # Train DT model
        dtc = core.dt()
        model = dtc.train_DT(train_data, train_label)
        # Evaluate the DT model
        print('========Processing %s DT model testing===============' % KPI_name)
        pred = dtc.test_DT(model, test_data)
        # Save the model
