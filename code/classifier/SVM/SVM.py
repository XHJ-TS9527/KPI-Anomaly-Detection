...
class main_activity():
    ...
    def main(self):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor: # multi CPUs processing
            executor.map(self.real_SVM, KPI_names)
        '''
        for each_KPI in KPI_names: # one CPU processing
            self.real_SVM(each_KPI)
        '''
        ...
        eval_file = open('SVM_PCA_evaluation.txt', 'wt')
        eval_file.writelines(eval_text)
        eval_file.close()
        print('SVM+PCA Experiment Finished.')

    def real_SVM(self, each_KPI):
        ...
        print('========Processing %s SVM model training==============' % KPI_name)
        # PCA
        target_feature_num = min([80, train_data.shape[1]])
        pca = decomposition.PCA(n_components=target_feature_num)
        pca.fit(train_data)
        ...
        # Train SVM model
        svm = core.SVM()
        model = svm.train_SVM(train_data, train_label)
        # Evaluate the SVM model
        print('========Processing %s SVM model testing==============' % KPI_name)
        pred = svm.test_SVM(model, test_data)
        # Save the model
        plk.dump(model, open(self.model_path + '%s.model' % KPI_name, 'wb'))
        # write the prediction result, if need ROC curve
        testing_text = []
        for idx in range(len(pred)):
            testing_text.append('%d,%.10f\n' % (test_label[idx], pred[idx, 0]))
