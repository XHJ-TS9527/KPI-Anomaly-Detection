...
class main_activity():
    ...
    def main(self):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor: # multi CPUs processing
            executor.map(self.real_OC_SVM, KPI_names)
        '''
        for each_KPI in KPI_names: # one CPU processing
            self.real_OC_SVM(each_KPI)
        '''
        ...
        eval_file = open('OC_SVM_evaluation.txt', 'wt')
        eval_file.writelines(eval_text)
        eval_file.close()
        print('OC-SVM Experiment Finished.')

    def real_OC_SVM(self, each_KPI):
        ...
        print('========Processing %s OC-SVM model training==============' % KPI_name)
        # Train OC-SVM model
        svm = core.oc_svm()
        model = svm.train_oc_svm(test_data)
        # Evaluate the OC-SVM model
        print('========Processing %s OC-SVM model testing==============' % KPI_name)
        pred = svm.test_oc_svm(model, test_data)
        # Save the model
