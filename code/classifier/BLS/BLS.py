...
class main_activity():
    ...
    def main(self):
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:  # multi CPUs processing
            executor.map(self.real_BLS, KPI_names)
        '''
        for each_KPI in KPI_names: # one CPU processing
            self.real_BLS(each_KPI)
        
        # write the training time
        time_content = []
        for each_KPI in KPI_names:
            KPI_name = each_KPI.replace('\n', '')
            time_content.append('%s,%.4f\n' % (KPI_name, self.train_time[KPI_name]))
        train_time_file = open('BLS_train_time.csv', 'wt')
        train_time_file.writelines(time_content)
        ...
        eval_file = open('BLS_evaluation.txt', 'wt')
        eval_file.writelines(eval_text)
        eval_file.close()
        print('BLS Experiment Finished.')

    def real_BLS(self, each_KPI):
        KPI_name = each_KPI.replace('\n', '')
        # Feature selection
        ...
        print('========Processing %s BLS model training==============' % KPI_name)
        # Train BLS model
        model = core.broadnet()
        util = core.BLS()
        model, self.train_time[KPI_name] = util.train_BLS(model, train_data, train_label)
        # Evaluate the BLS model
        print('========Processing %s BLS model testing===============' % KPI_name)
        pred = util.test_BLS(model, test_data)
        # Save the model
